...
checkpointer = callbacks.ModelCheckpoint(filepath="logs/lstm2layer/checkpoint-{epoch:02d}.hdf5", verbose=1, save_best_only=True, monitor='val_acc', mode='max')
model.fit(X_train, y_train, batch_size=batch_size, validation_data=(X_test, y_test),nb_epoch=500, callbacks=[checkpointer])
model.save("logs/lstm2layer/lstm1layer_model.hdf5") 
'''
